...
TEST( HC_Sample, trival_get )
{
	HC_Sample hc;
	hc.Config( 12, 34 );
	EXPECT_DOUBLE_EQ( hc.GetTimeLimit(), 12 );
	EXPECT_EQ( hc.GetMaxIteration(), 34 );
}
...
TEST( SimulatedAnnealing, trival_get )
{
	SA_Sample sa;
	sa.Config( 12, 34 );
	EXPECT_DOUBLE_EQ( sa.GetTimeLimit(), 12 );
	EXPECT_EQ( sa.GetMaxIteration(), 34 );
}
...
TEST( SA_TSP_sample, sa2hc )
{
	int n = 50;
	SA_TSP_sample sa( n );
	auto hc = HillClimbFromSimulatedAnnealing<SA_TSP_sample>( sa );
	hc.Config( 3, 10000, true, ( int )decltype( hc )::tState::kAcceptSolution );
	ASSERT_TRUE( hc.Execute( 0 ) );
